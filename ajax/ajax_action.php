...
function wqnew_entry_callback_function() {
  global $wpdb;
  $wpdb->get_row( "SELECT * FROM `theme_crud` WHERE `ThemeFormation` = '".$_POST['wqThemeFormation']."' AND `description` = '".$_POST['wqdescription']."' ORDER BY `id` DESC" );
  if($wpdb->num_rows < 1) {
    $wpdb->insert("theme_crud", array(
...
function wqedit_entry_callback_function() {
  global $wpdb;
  $wpdb->get_row( "SELECT * FROM `theme_crud` WHERE `ThemeFormation` = '".$_POST['wqThemeFormation']."' AND `description` = '".$_POST['wqdescription']."' AND `id`!='".$_POST['wqentryid']."' ORDER BY `id` DESC" );
  if($wpdb->num_rows < 1) {
    $wpdb->update( "theme_crud", array(
      "ThemeFormation" => $_POST['wqThemeFormation'],
      "description" => $_POST['wqdescription'],
      "updated_at" => time()
